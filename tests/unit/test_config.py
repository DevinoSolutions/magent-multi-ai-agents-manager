...
class TestAccountRoutingSettings:
    """``settings.accounts`` -- the schema-v4 block, which ships DARK.

    The headline property is the first test: a config that never mentions
    accounts parses exactly as it did before the block existed, with routing
    off. Everything else here is about a mistyped field being visible without
    being fatal, which is this module's standing doctrine.
    """

    def test_absent_block_parses_to_routing_off(self, tmp_config):
        path = tmp_config({"projects": [{"path": "api"}]})
        accounts = load_config(path).settings.accounts
        assert accounts.enabled is False
        assert accounts.soft_threshold == 85.0
        assert accounts.hard_threshold == 95.0
        assert accounts.on_limit == "move-if-reset>2h"
        assert accounts.stale_after_s == 900.0
        assert accounts.status_left is True
        assert accounts.per_account == {}

    def test_every_knob_round_trips(self, tmp_config):
        path = tmp_config(
            {
                "settings": {
                    "accounts": {
        ...
        assert accounts.soft_threshold == 70.0
        assert accounts.hard_threshold == 90.5
        assert accounts.on_limit == "wait"
        assert accounts.stale_after_s == 60.0
        assert accounts.status_left is False
        assert accounts.per_account["13"].klass == "fable"
        assert accounts.per_account["13"].exclude is True
        # Absent inside an override means "inherit the global one", which is
        # why it is "" and not the fallback policy.
        assert accounts.per_account["13"].on_limit == ""
        assert accounts.per_account["19"].on_limit == "move"
        assert accounts.per_account["19"].exclude is False

    @pytest.mark.parametrize(
        "value", ["wait", "move", "move-if-reset>2h", "move-if-reset>0.5h", "MOVE"]
    )
    def test_recognised_on_limit_policies_survive_verbatim(self, tmp_config, value):
        # Verbatim, including the case the user typed: routing.parse_on_limit
        # lowercases on its own, and rewriting a config value here would make
        # `magent config show` disagree with the file.
        path = tmp_config(
            {
                "settings": {"accounts": {"onLimit": value}},
                "projects": [{"path": "api"}],
            }
        )
        assert load_config(path).settings.accounts.on_limit == value

    @pytest.mark.parametrize("value", ["move-if-reset", "move-if-reset>2", "drift", ""])
    def test_an_unknown_on_limit_degrades_to_wait_with_a_warning(
        self, tmp_config, capsys, value
    ):
        # `wait` and NOT the default policy: a misspelled onLimit must never be
        # read as permission to recreate a live session.
        path = tmp_config(
            {
                "settings": {"accounts": {"onLimit": value}},
                "projects": [{"path": "api"}],
        ...
        assert load_config(path).settings.accounts.on_limit == "wait"
        err = capsys.readouterr().err
        assert "settings.accounts.onLimit" in err
        assert "wait" in err

    def test_an_empty_override_policy_inherits_rather_than_waiting(
        self, tmp_config, capsys
    ):
        # The asymmetry with the case above, and the reason `inheritable`
        # exists: inside perAccount there IS a global policy to fall back to,
        # so an empty value is an absent override and not a broken one.
        path = tmp_config(
            {
                "version": SCHEMA_VERSION,
                "settings": {"accounts": {"perAccount": {"13": {"onLimit": ""}}}},
                "projects": [{"path": "api"}],
            }
        )
        assert load_config(path).settings.accounts.per_account["13"].on_limit == ""
        assert capsys.readouterr().err == ""

    def test_an_unknown_accounts_key_warns(self, tmp_config, capsys):
        path = tmp_config(
            {
                "settings": {"accounts": {"softThresold": 70}},
                "projects": [{"path": "api"}],
            }
        )
        load_config(path)
        assert "settings.accounts.softThresold" in capsys.readouterr().err

    def test_an_unknown_key_inside_an_override_warns_under_its_account_id(
        self, tmp_config, capsys
    ):
        path = tmp_config(
            {
                "settings": {"accounts": {"perAccount": {"13": {"klass": "fable"}}}},
                "projects": [{"path": "api"}],
            }
        )
        load_config(path)
        assert "settings.accounts.perAccount.13.klass" in capsys.readouterr().err

    def test_an_account_id_is_never_itself_warned_about(self, tmp_config, capsys):
        # The one key space in this schema deliberately NOT validated: the keys
        # are ccswap account ids, which come from the user's ccswap store and
        # not from magent. Warning about them would warn about every account
        # magent has simply never heard of.
        path = tmp_config(
            {
                "version": SCHEMA_VERSION,
                "settings": {
                    "accounts": {
                        "perAccount": {"totally-made-up-id": {"exclude": True}}
                    }
                },
                "projects": [{"path": "api"}],
            }
        ...
        cfg = load_config(path)
        assert cfg.settings.accounts.per_account["totally-made-up-id"].exclude is True
        assert capsys.readouterr().err == ""

    def test_a_mistyped_block_degrades_to_defaults(self, tmp_config):
        path = tmp_config(
            {
        ...
        assert load_config(path).settings.accounts.enabled is False

    def test_a_non_object_override_is_dropped_not_defaulted(self, tmp_config):
        # An override that degraded to AccountOverride() would silently
        # un-exclude an account the user meant to keep out of routing.
        path = tmp_config(
            {
                "settings": {"accounts": {"perAccount": {"13": "exclude"}}},
                "projects": [{"path": "api"}],
            }
        )
        assert load_config(path).settings.accounts.per_account == {}


class TestProjectAccountFields:
    """``account`` (the pin) and ``modelClass`` (the class override)."""

    def test_both_default_to_none(self, tmp_config):
        path = tmp_config({"projects": [{"path": "api"}]})
        project = load_config(path).projects[0]
        assert project.account is None
        assert project.model_class is None

    def test_a_pin_and_a_class_round_trip(self, tmp_config):
        path = tmp_config(
            {"projects": [{"path": "api", "account": "13", "modelClass": "fable"}]}
        )
        project = load_config(path).projects[0]
        assert project.account == "13"
        assert project.model_class == "fable"

    @pytest.mark.parametrize(
        ("written", "parsed"),
        [("FABLE", "fable"), ("  standard  ", "standard"), ("Standard", "standard")],
    )
    def test_a_class_is_normalised(self, tmp_config, written, parsed):
        path = tmp_config({"projects": [{"path": "api", "modelClass": written}]})
        assert load_config(path).projects[0].model_class == parsed

    def test_an_unknown_class_degrades_to_none_with_a_warning(self, tmp_config, capsys):
        # None means "infer it", which is the safe answer -- and NOT the same
        # as "standard", which is what the planner falls back to on its own.
        path = tmp_config({"projects": [{"path": "api", "modelClass": "opus"}]})
        assert load_config(path).projects[0].model_class is None
        err = capsys.readouterr().err
        assert "projects[0].modelClass" in err
        assert "opus" in err

    def test_a_pin_is_never_validated_against_a_list(self, tmp_config, capsys):
        # Account ids come from ccswap. The planner warns about a pin it cannot
        # resolve at PLAN time, where it actually knows the accounts; the
        # loader would only be guessing.
        path = tmp_config(
            {
                "version": SCHEMA_VERSION,
                "projects": [{"path": "api", "account": "nope"}],
            }
        )
