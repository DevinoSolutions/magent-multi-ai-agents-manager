...
def find_in_snapshot(
    snap: dict[str, object], key: str, mode: str = "magent-name"
) -> object | None:
    """The window handle ``place_windows`` would resolve for ``key``, or None.

    The public face of the very matcher tiling places with, so a caller that
    needs the handle itself (cli/attach's post-tiling geometry nudge) cannot
    drift from how tiling found the same window moments earlier -- badge-proof
    ``magent-name`` matching included.
    """
    return _lookup(snap, key, mode)
...
def window_open(snap: dict[str, object], key: str, mode: str = "magent-name") -> bool:
    ...
    Same matcher as ``find_in_snapshot``/``place_windows``, so a caller asking
    "is this window already up?" before spawning one (cli/attach's spawn loops,
    mirroring launch.py's ``_is_running``) cannot drift from how tiling will
    later find it.
    """
